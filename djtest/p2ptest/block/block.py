...
import hashlib
import re
from p2ptest.synchronization import synchronization
...
class Block:
    ...
    def From(node,info):
        ...
        response = p2p.p2p.Node.send(node,SERVICE * SYNCHRONIZATION + FROM,info)
    ...
    def To(self,node):
        ...
        response = p2p.p2p.Node.send(node,SERVICE * SYNCHRONIZATION + TO,self.pb2)
